    ...
    if (name.indexOf('*') != -1) {
        ...
        for (var i in metrics) {
            ...
            if (metric.children.length == 0) {
                /* Can delete it. Tell the metric to clean itself up */
                this.metrics[metric.name].remove();
        ...
        if (m == null) {
            ...
            /* Tell the metric to clean itself up */
            this.metrics[name].remove();
...
/**
 * Get data from the metric for the range [start, end] asynchronously
 *
 * If the metric doesn't exist, an empty array is returned, but that metric will 
 * not be instantiated.
 */
DB.prototype.get = function(name, start, end, cb) {
    var m = this.metrics[name];
    if (m === undefined) {
