...
function watcher(parent, metric) {
    ...
    /* Add this alarm to the metric */
    metric.alarms.push(parent);
    console.log(metric.name + ': ' + metric.alarms.length);
...
function alarm(db, pattern, condition, duration, callback) {
    ...
    /* Add self to the database's list of alarms */
    db.alarms.push(this);
}
...
alarm.prototype.off = function() {
    ...
    for (var i in this.watchers) {
        ...
        /* Also, remove this alarm from the metric */
        var index = this.watchers[i].metric.alarms.indexOf(this);
        if (index != -1) {
            this.watchers[i].metric.alarms.splice(index, 1);
        }
        delete this.watchers[i];
    }
    /* Oh, and delete all of our watchers */
    delete this.watchers;
    /* Remove this alarm from the database */
    var index = this.db.alarms.indexOf(this);
    if (index != -1) {
        this.db.alarms.splice(index, 1);
    }
}
